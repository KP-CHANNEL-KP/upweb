...
export default function PremiumPage() {
  const [selectedKey, setSelectedKey] = useState<Plan | null>(null);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        console.error(
          'Status စစ်ဆေးရာတွင် အမှားဖြစ်ပွားသည်'
        );
      }
    }, 3000);
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-gray-100 p-4 md:p-8">
      {error && (
        <div className="text-red-500 text-center mb-4 p-2 bg-red-900/20 rounded-lg">
          {error}
        </div>
      )}
      ...
      <div className="mt-12 text-center">
        ...
        {result?.access_url && (
          <div className="mt-6 p-4 bg-emerald-900/20 border border-emerald-500 rounded-xl max-w-lg mx-auto">
            ...
            <code className="block bg-black p-3 mt-2 break-all text-sm rounded">
              {result.access_url}
            </code>
          </div>
        )}
